  ...
  <div class="space-y-6">
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
      <div>
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400">Tab mengikuti data Jenis Kas, lengkap dengan filter dan
          pencarian.</p>
      </div>
      <BaseButton text="Export Excel" variant="primary" :fullWidth="false" icon="lucide:download"
        @click="openExportModal" :disabled="!transactions.length" />
    </div>
    ...
    <div class="rounded-xl border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 shadow-sm">
      <div class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-8">
        <div class="xl:col-span-2">
          <label
            class="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Cari</label>
          <input v-model="searchInput" type="text" placeholder="Cari uraian atau Jenis Kas..."
        ...
        <div>
          <label class="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Jenis
            Kas</label>
          <select v-model="activejenisKasId"
            class="w-full rounded-lg border border-gray-300 dark:border-gray-600 px-3 py-2 text-sm dark: outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400">
        ...
        <div>
          <label
            class="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Kategori</label>
        ...
        <div>
          <label
            class="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Tanggal
            Mulai</label>
    ...
    <div class="rounded-xl border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 p-2 shadow-sm">
      <div class="flex flex-wrap gap-2">
        <button type="button" @click="selectJenisKasTab('all')" :class="[
          ...
          <span class="ml-2 rounded-full px-2 py-0.5 text-xs"
            :class="activejenisKasId === 'all' ? 'bg-white/20' : 'bg-gray-200 dark:bg-gray-600'">{{ meta?.totalItems ||
            0 }}</span>
        </button>

        <button v-for="tab in JenisKasTabs" :key="tab.id" type="button" @click="selectJenisKasTab(tab.id)" :class="[
          'rounded-lg px-4 py-2 text-sm font-medium transition-colors',
    ...
    <div class="grid grid-cols-1 gap-3 md:grid-cols-3">
      ...
      <div class="rounded-xl border border-blue-200 bg-blue-50 p-4">
        ...
        <p class="mt-1 text-xl font-bold text-blue-700"
          :class="summary.netBalance < 0 ? 'text-red-600' : 'text-blue-700'">{{ formatCurrency(summary.netBalance) }}
        </p>
      </div>
    </div>

    <div
      class="overflow-hidden rounded-xl border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm">
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
          <thead
            class="bg-gray-50 dark:bg-gray-700/50 text-xs uppercase tracking-wide text-gray-600 dark:text-gray-400">
            <tr>
          ...
          <tbody class="divide-y divide-gray-100">
            ...
            <tr v-else-if="transactions.length === 0">
              <td colspan="8" class="px-6 py-8 text-center text-sm text-gray-500 dark:text-gray-400">Tidak ada data yang
                cocok dengan filter.</td>
            </tr>
            <tr v-else v-for="(row, index) in transactions" :key="row.id"
              ...
              <td class="px-6 py-4 text-right font-semibold"
                :class="getDebit(row) > 0 ? 'text-emerald-600' : 'text-gray-400'">
                {{ formatCurrency(getDebit(row)) }}
              </td>
              <td class="px-6 py-4 text-right font-semibold"
                :class="getKredit(row) > 0 ? 'text-red-600' : 'text-gray-400'">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                <div class="flex items-center gap-2">
                  <div
                    class="w-6 h-6 rounded-full bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 flex items-center justify-center text-xs font-bold shrink-0 overflow-hidden">
                    <img v-if="row.user?.fotoProfile" :src="resolveAssetUrl(row.user.fotoProfile)" alt="Profile"
      ...
      <BasePagination v-if="meta && meta.totalPages > 1" v-model="currentPage" :meta="meta" />
    </div>

    <BaseModal v-model="showExportModal" title="Pilih Format Export" icon="lucide:file-spreadsheet" type="info"
      confirmText="Export Sekarang" @confirm="handleExportConfirm">
      <div class="space-y-4">
        <div>
          <label class="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Mode
            Export</label>
        ...
        <div>
          <label class="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Jenis
            Kas</label>
          <select v-model="exportJenisKasId"
            class="w-full rounded-lg border border-gray-300 dark:border-gray-600 px-3 py-2 text-sm dark: outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400">
...
import { exportLaporanKasToExcel } from '~/application/utils/exportLaporanKas';
...
const handleExportConfirm = async () => {
  ...
  exportLaporanKasToExcel(
    exportMode.value,
    exportYear.value,
    exportMonth.value,
    allFilteredRows as ITransaksi[]
  );
};
